...
import os
model_path = "/Users/tiankongzhicheng/Desktop/UOA_Assignment/CodeLlama-7b-Instruct-hf"
...
if torch.backends.mps.is_available():
    ...
    print("MPS device is available. Attempting to load model and move to MPS...")
    try:
        model = AutoModelForCausalLM.from_pretrained(
            model_path,
            torch_dtype=torch.float16
        )
        print("Model loaded to CPU RAM, now moving to MPS...")
        model.to(device)
        print("Model successfully moved to MPS!")
    except Exception as e:
        print(f"Error during MPS loading/moving: {e}")
        # If MPS fails, try falling back to CPU
        print("Falling back to CPU loading due to MPS error.")
        device = torch.device("cpu")
        try:
            model = AutoModelForCausalLM.from_pretrained(
                model_path,
                torch_dtype=torch.float16 # Still try float16 if memory allows
            )
            model.to(device) # Ensure on CPU
            print("Model successfully loaded to CPU!")
        except Exception as e_cpu:
            print(f"Error loading model to CPU after MPS fallback: {e_cpu}")
elif torch.cuda.is_available(): # Keep CUDA option just in case
    device = torch.device("cuda")
    print("CUDA device is available. Attempting to load model and move to CUDA...")
    try:
        model = AutoModelForCausalLM.from_pretrained(
            model_path,
            torch_dtype=torch.float16, # For NVIDIA GPU, float16 usually works well
            device_map="auto" # On CUDA, device_map="auto" typically works well
        )
        print("Model successfully loaded to CUDA with device_map='auto'!")
    except Exception as e:
        print(f"Error during CUDA loading: {e}")
        # Fallback logic for CUDA loading failure (e.g., to CPU) can be added similarly
else:
    device = torch.device("cpu")
    print("Neither MPS nor CUDA available, setting device to CPU. Attempting to load model to CPU...")
    try:
        model = AutoModelForCausalLM.from_pretrained(
            model_path,
            torch_dtype=torch.float16 # On CPU, can also try float16 if memory is sufficient
        )
        model.to(device) # Ensure model is on CPU
        print("Model successfully loaded to CPU!")
    except Exception as e:
        print(f"Error loading model to CPU: {e}")
...
Let's think step by step:
    ...
    if "yes" in response.lower():
        ...
        # This part can be adjusted based on actual output from CodeLlama
...
def main():
    ...
    for i, item in enumerate(dataset):
        ...
        if (i + 1) % 10 == 0:
            temp_output_file = "data_output_CodeLlama7b_Reasoning.json"
            try:
    ...
    output_file = "data_output_CodeLlama7b_Reasoning.json"
    try:
